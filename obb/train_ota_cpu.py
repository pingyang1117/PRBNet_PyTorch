...
from utils.loss import ComputeLoss, ComputeLossOTA_CPU
...
#            g1.append(v.weight)
    ...
    compute_loss_ota = ComputeLossOTA_CPU(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss_ota([p.to('cpu') for p in pred], targets.to('cpu'), imgs)  # loss scaled by batch_size
                loss, loss_items = loss.to(device), loss_items.to(device)
                if RANK != -1:
