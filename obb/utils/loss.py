...
class UFocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        p_t = true * pred_prob + (1 - true) * (1 - pred_prob)
        alpha_factor = true * self.alpha + (1 - true) * (1 - self.alpha)
        modulating_factor = (torch.abs(true - pred_prob)/torch.abs(pred_prob)) ** self.gamma
        loss *= alpha_factor * modulating_factor
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #riou = = bbox_iou(pbox.T, tbox[i], ps)   # calculate riou
...
class ComputeLossAuxOTA:
    ...
    def find_3_positive(self, p, targets):
        ...
        # indices[i] = (bi, ai, gy_i, gx_i, tbox)
        return indices, anch
# ComputeLoss function for iBin head with simOTA ft circular smooth label
#   ref: https://github.com/WongKinYiu/yolov7/blob/main/utils/loss.py
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            indices.append((b, a, gj.clamp_(0, feature_wh[1] - 1), gi.clamp_(0, feature_wh[0] - 1),
                            tbox))  # image, anchor, grid indices
            anch.append(anchors[a])  # anchors
